...
class Memory(nn.Module):
    def __init__(self, opt, vocab_size):
        ...
        if not self.lstm:
            ...
            self.conv1 = nn.Conv2d(1, self.mem_size//3, (self.kernel_sizes[0], self.word_vec_size), padding=(self.kernel_sizes[0]//2, 0))
            self.conv2 = nn.Conv2d(1, self.mem_size//3, (self.kernel_sizes[1], self.word_vec_size), padding=(self.kernel_sizes[1]//2, 0))
            self.conv3 = nn.Conv2d(1, self.mem_size//3, (self.kernel_sizes[2], self.word_vec_size), padding=(self.kernel_sizes[2]//2, 0))
    ...
    def load_pre_word_vecs(self):
        if self.pre_word_vecs is not None:
            ...
            pretrained = np.load(self.pre_word_vecs, allow_pickle=True)
