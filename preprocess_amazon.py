...
GLOVE_PATH = "/research/king3/hpchan/datasets/glove_6B/glove.6B.300d.txt"
...
class PreProcess(object):
    ...
    def makeVocab(self, filepath):
        ...
        #' ', 'RAW Max len', max_len
        #print
        #' ', 'RAW Max vocabulary size', len(self.token2id)

        # cPickle.dump(self.token2id.keys(),open('data/vocab.pkl', 'wb'))

        return {'size': len(self.token2id), 'token2id': self.token2id, 'id2token': self.id2token,
    ...
    def makeData(self, filepath):
        ...
        for line in lines:
            ...
            if not sentence in unique_sent:
                words = sentence.split(' ')
                s_data.append(torch.LongTensor(
                    [self.token2id[word] if word in self.token2id else constant.UNK_ID for word in words]))
        ...
        if self.sorted_size:
            #print
            #'  Sorting sentence by size'
            _, perm = torch.sort(torch.Tensor(sizes))
            s_data = [s_data[idx] for idx in perm]
            l_data = [l_data[idx] for idx in perm]

        #print
        #'  Got total', len(s_data), 'samples'

        # print '---', s_data[0]

        return s_data, l_data

    def wordvec(self, time_stamp):
        lines = codecs.open(GLOVE_PATH, 'r', 'utf-8').readlines()
        wordvec = {}
        for line in lines:
            l = line.strip().split()
            word = l[0]
        ...
        for i in range(len(self.token2id)):
            ...
            if word in wordvec:
                count += 1
            vec = wordvec[word] if word in wordvec else np.random.uniform(-0.1, 0.1, size=(1, 300))
            wv[i] = vec
        #print
        #' ', count, 'found in Glove'
        file_name = 'wv_noex2_%s.npz' % time_stamp if self.binary else self.save_data + 'wv_noex5_%s.npz' % time_stamp

        #print
        #' Save to', file_name
        wv.dump(join(self.save_data, file_name))
        print("save to {}".format(join(self.save_data, file_name)))
    ...
    def process(self):
        ...
        #print
        #'* Saving data to \'' + self.save_data + outfile + '...'
        save_data = {'vocab': vocab,
                     'train': train,
                     'valid': valid,
                     'test': test}
        torch.save(save_data, join(self.save_data, outfile))
        print("save to {}".format(join(self.save_data, outfile)))
        #print
        #'* OK.'

        # get pre-trained wordvec
        #print
        #'* Dump pre-trained wordvec...'
...
if __name__ == "__main__":
    ...
    train_src = join(args.data_dir, 'sent_class_train.txt')
    valid_src = join(args.data_dir, 'sent_class_val.txt')
    test_src = join(args.data_dir, 'sent_class_test.txt')

    p = PreProcess(train_src=train_src, valid_src=valid_src, test_src=test_src, corpus_src=train_src, vocab_size=50000, shuffle=True, binary=False, save_data=args.data_dir)
    p.process()
