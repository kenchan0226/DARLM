...
from utils.utils_glue import balanced_acc_and_macro_f1
...
def merge_res(res_data, compute_balanced_score=False): # avg-max
    count = 0.0
    pred_ids = []
    label_ids = []
    for i in range(len(res_data[0])):
        ...
        label_ids.append(res_data[m_index][i][2])
        if pred_index == res_data[m_index][i][2]:
            count += 1
    pred_ids = np.array(pred_ids)
    label_ids = np.array(label_ids)
    balanced_cf_result = balanced_acc_and_macro_f1(pred_ids, label_ids)
    balanced_acc = balanced_cf_result['acc']
    macro_f1 = balanced_cf_result['f1']

    return count / len(res_data[0]), balanced_acc, macro_f1
...
def merge_res5(res_data, pred_chooses, k=None):
    ...
    def val(k):
        ...
        return acc, oracle_acc, balanced_acc, macro_f1

    best_th, best_acc, oracle_acc = 0, 0, 0

    if k:
        best_acc, oracle_acc, balanced_acc, marco_f1 = val(k)
    else:
        for th in range(100):
            threshold = float(th)/100
            last_acc, oracle_acc, balanced_acc, marco_f1 = val(threshold)
            if last_acc > best_acc:
                best_acc = last_acc
                best_th = threshold

    return best_acc, oracle_acc, best_th, balanced_acc, marco_f1
